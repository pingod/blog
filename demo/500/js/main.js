...
var Cathectic = function(){
	...
	*/
	var chooseTpl = function(N){
		...
		for(var i = 1; i <= n;="" i++){="" str.push("<span=""><strong>" + i + "</strong><em>8</em>");
		}
	...
	return {
		...
		*/
		_ceateItemListSelect: function(num){
			...
			for(var i = num; i <= config['totalnum']="" -="" 1;="" i++){="" arr.push("<option="" value="" + i + "">机选" + i + "个");
			}
		...
		_dealText: function(){
			...
			for(var i = 0; i < val.length; i++){
				...
				for(var j = 0; j < arrTmp.length; j++){
					...
					if(valTmp > config['totalNum'] || valTmp <= 1="" 0){="" self.alertbox("您好，您输入的第"="" +="" (i="" 1)="" "行的格式有误="" [输入数值错误，数值区间应该是="" -="" "+="" config["totalnum"]="" +"]="" ，请检查！");="" return="" false;="" }="" if(hashtable[arrtmp[j]]="=" null){="" hashtable[arrtmp[j]]="1;" hashtable.length="" 个数错误，应该是="" curnum="" 个="" if(arrtmp.length="" !="curNum){" [个数错误，应该是"="" "个]="" 存在重复="" if(hashtable.length="" <="" arrtmp.length){="" [存在重复]="" console.log(hashtable);="" $.map(val,="" function(ctt){="" if($.trim(ctt)="" )="" {="" curres="[ctt.replace(/\s+|,/g," ",").split(",")];="" var="" str="self._getItemTpl();" $(".cathectic-resluts-list").append(str);="" });="" self._updatetotalmoney();="" self.resetdom();="" $num="$(".CSR_info" span").eq(0),="" $money="$(".CSR_info" span").eq(1)="" $(".csr_btn").off().removeclass("csr_btn_ok");="" $num.text(0);="" $money.text("0.00");="" },="" **="" *="" @description="" 主要是对="" dom="" 的操作，修改一些数字。显示当前的一些参数="" _setmoney:="" function(){="" res="this._calcMoney()" ,="" span").eq(0)="" $win="$(".CSR_get" $get="$(".CSR_get" span").eq(1);="" self="this;" if(!res="" ||="" choosetype="=" "text"="" &&="" $(".cathectic-main-choose-text="" textarea").val()="=" ""){="" else="" if(choosetype="=" return;="" csrbtncb="null;" 单次绑定，点击选好了之后，不能再点击，并且清空选中内容="" $(".csr_btn").off("click").addclass("csr_btn_ok").one("click",="" evt.preventdefault();="" "text"){="" self._dealtext();="" tmp="self._checkIsUse();" if(tmp.length=""> 0 && chooseType == "dingwei"){
		...
		*/
		_getItemTpl: function(data){
			var tpl = ['<div data-num="<%num%>" data-type="<%type%>">',
							...
							'<span>[<%type%>]</%type%></span>',
							'<%content%>',
							'(<i><%num%></%num%></i>注<i><%money%></%money%></i>元)',
						'</%content%></div>'].join("");
